...
class NetG(nn.Module):
    ...
    def forward(self, x, masked_input=None):
        if self.opt.use_context_pred and masked_input is not None:
            ...
            reconstructed_image = self.decoder(self.encoder1(masked_input))
            latent_o = self.encoder2(reconstructed_image)
            gen_imag = reconstructed_image
        else:
            # 기존 방식 유지
            latent_i = self.encoder1(x)
        ...
        # 추가: 원본 latent_i를 사용하여 복원된 이미지 반환
        reconstructed_from_latent = self.decoder(latent_i)
        
        # 원본 이미지를 통해 복원된 잠재 벡터 추가 계산
        latent_orig = self.encoder2(self.decoder(latent_i))
        
        return gen_imag, reconstructed_from_latent, latent_orig, latent_i, latent_o
